...
**Za šta se koristi:** Renderuje React komponente u test okruženju i daje ti alate da ih testiraš iz perspektive korisnika (kroz DOM upite).

`@testing-library/jest-dom` - Custom matchers to simplify assertions
**Šta je:** Set dodatnih matcher-a za asertacije nad DOM-om (radi i uz Vitest, uprkos nazivu “jest-dom”).
**Za šta se koristi:** Čini asertacije čitljivijim i prirodnijim: toBeInTheDocument, toHaveTextContent, toBeDisabled, toHaveAttribute, toBeVisible, toHaveStyle, itd.

`jsdom` - Simulates a browser-like environment for tests to run in Node.js
**Šta je:** Implementacija DOM-a u Node okruženju (headless, bez pravog browsera).
**Za šta se koristi:** Omogućava da se React komponente renderuju i testiraju u Node-u kao da smo u browseru. Vitest koristi environment: 'jsdom' da bi kreirao window, document, HTMLElement, evente itd.

`@testing-library/user-event` - Simulates user interactions (clicks, typing, etc.) in tests
**Šta je:** Biblioteka za realistične korisničke interakcije u testovima.
**Za šta se koristi:** Simulira prave akcije korisnika (tipkanje, klikovi, dupli klik, tab navigacija, selekcija teksta, pritisci tastera, držanje tastera, nalepiti tekst…), asinkrono i u skladu sa browser ponašanjem.
